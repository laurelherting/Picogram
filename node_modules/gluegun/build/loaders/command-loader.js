...
function loadCommandFromFile(file, options) {
    ...
    command.commandPath = (options.commandPath || ramda_1.last(file.split('commands' + path.sep)).split(path.sep)).map(function (f) { return ([command.name + ".js", command.name + ".ts"].includes(f) ? command.name : f); });
